...
type fakeClient struct {
	...
	// eventNames records what each stored event was called, so the naming fallback can be checked.
	eventNames []string
}
...
func (c *fakeClient) StoreEvent(_ context.Context, in *hippo.Event, _ ...grpc.CallOption) (*hippo.StoreEventResponse, error) {
	...
	c.eventNames = append(c.eventNames, in.GetName())
...
// TestEventNamesFollowTheTrace checks the two ways an event gets its name: a session generated by
// the realistic vocabulary carries one, and a synthetic session does not, leaving the replay to
// name it after its group.
func TestEventNamesFollowTheTrace(t *testing.T) {
	for _, tc := range []struct {
		name       string
		vocabulary trace.Vocabulary
		want       string
	}{
		{name: "synthetic falls back to the group", vocabulary: trace.VocabSynthetic, want: "working session"},
		{name: "realistic carries its own name", vocabulary: trace.VocabRealistic, want: "work on"},
	} {
		t.Run(tc.name, func(t *testing.T) {
			tr := testTrace(t, 60)

			if tc.vocabulary == trace.VocabRealistic {
				cfg := tr.Config
				cfg.Vocabulary = trace.VocabRealistic

				var err error

				if tr, err = trace.Generate(cfg); err != nil {
					t.Fatalf("generating the realistic trace: %v", err)
				}
			}

			c := newFake()

			if _, err := New(c, tr, Config{
				SimDaysPerWallMinute: 100000,
				Workers:              4,
				Tick:                 time.Millisecond,
				Events:               true,
			...
			if len(c.eventNames) == 0 {
				t.Fatal("no events were stored")
			}

			for _, v := range c.eventNames {
				if !strings.Contains(v, tc.want) {
					t.Errorf("event named %q, wanted one containing %q", v, tc.want)
				}
			}
		})
	}
